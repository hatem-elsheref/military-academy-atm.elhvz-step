...
<?php endif;?>
      ...
      <h1>Prepare Atm Elhvz Step - <span class="text-danger">Relatives</span></h1>
      <hr>
        <form method="get" action="/relatives.php">
             <div class="row">
                <div class="col">
  ...
  if(isset($page) && isset($number_of_pages) && isset($_GET['date'])):
    $prev = $page == 1 ? '#' : '/relatives.php?date=' . $_GET['date'] . '&page=' . ($page - 1);
    $next = $page == $number_of_pages ? '#' : '/relatives.php?date=' . $_GET['date'] . '&page=' . ($page + 1);
?>
  ...
  <ul class="pagination">
    ...
    <li  class="page-item <?= $page == $p ? 'active' : ''?>" ><a class="page-link" href="<?="/relatives.php?date=" . $_GET['date'] . '&page=' . $p?>"><?=$p?></a></li>
    <?php endforeach?>
    <?php endif;?>
    <li class="page-item">
